...
```mermaid
graph TD
    A["Preparación<br/>(1 semana antes)"] --> B["Armado del stand<br/>(día de la feria)"]
    B --> C["Venta activa<br/>(durante el evento)"]
    C --> D["Cierre y<br/>seguimiento<br/>(post-feria)"]

    A --> A1["Stock seleccionado<br/>Packaging<br/>Cartelería"]
    B --> B1["Display<br/>Precios visibles<br/>QR de cobro"]
    C --> C1["Atención al cliente<br/>Cobro con MP/NFC<br/>Captura de seguidores"]
    D --> D1["Conteo de stock<br/>Análisis de ventas<br/>Mensaje a contactos"]
