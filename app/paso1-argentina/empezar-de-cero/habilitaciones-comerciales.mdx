...
```mermaid
graph TD
    A["Que tipo de venta<br/>haces?"] --> B{"Solo online?"}
    B -- Si --> C{"Vendes productos<br/>regulados?"}
    C -- No --> D["No necesitas<br/>habilitacion municipal<br/>(Solo ARCA + IIBB)"]
    C -- Si --> E["Necesitas permisos<br/>especificos segun<br/>el producto"]
    B -- No --> F{"Tenes local<br/>fisico?"}
    F -- Si --> G["Necesitas habilitacion<br/>municipal + ARCA + IIBB"]
    F -- No --> H["Showroom / deposito<br/>consulta municipio"]
```
