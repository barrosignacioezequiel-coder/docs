...
```mermaid
graph TD
    A["Fábrica<br/>(consignante)"] -->|"1) Entrega productos"| B["Vos<br/>(consignatario)"]
    B -->|"2) Vendés al consumidor"| C["Cliente<br/>final"]
    C -->|"3) Pagó el producto"| B
    B -->|"4) Liquidás ventas (periódicamente)"| A
    B -->|"5) Devolvés lo no vendido"| A
