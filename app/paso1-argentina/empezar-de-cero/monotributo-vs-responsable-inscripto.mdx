...
```mermaid
graph TD
    A["Empezás con Monotributo"] --> B{"Tu facturación anual<br/>supera el límite<br/>de tu categoría?"}
    B -- No --> C["Seguís en Monotributo<br/>(recategorizate si corresponde)"]
    B -- Sí --> D{"Podés subir<br/>a la categoría<br/>siguiente?"}
    D -- Sí --> E["Recategorizate<br/>(sube de categoría)"]
    D -- No, ya estás en K --> F["Pasá a<br/>Responsable Inscripto"]
    C --> G["Revisá cada 6 meses"]
    E --> G
    F --> H["Contratá un contador<br/>antes de hacer el cambio"]
```
