...
```mermaid
graph TD
    A["Detectás un competidor<br/>más barato"] --> B{"¿Es el mismo producto<br/>exactamente?"}
    B -- "No" --> C["No es competencia directa<br/>Diferenciá tu publicación"]
    B -- "Sí" --> D{"¿Vende de la<br/>misma fábrica?"}
    D -- "No sé" --> E["Investigá: preguntá<br/>en la fábrica"]
    D -- "Sí" --> F{"¿Cuántas ventas tiene?"}
    D -- "No, importa de China" --> G["Competencia diferente:<br/>competí por calidad nacional"]
    F -- "Pocas (0-10)" --> H["Probablemente está empezando<br/>Monitoreá pero no reacciones"]
    F -- "Muchas (50+)" --> I["Competencia real:<br/>evaluá tu estrategia"]
```
