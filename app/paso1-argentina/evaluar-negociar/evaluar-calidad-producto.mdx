...
```mermaid
graph TD
    A["Recibís el lote"] --> B["Comparás con<br/>muestra guardada"]
    B --> C{"La calidad<br/>coincide?"}
    C -- Sí --> D["Todo bien<br/>Guardar muestra<br/>de este lote"]
    C -- No --> E["Documentar<br/>con fotos/videos"]
    E --> F["Contactar<br/>a la fábrica"]
    F --> G{"Ofrecen<br/>solución?"}
    G -- Sí --> H["Aceptar si<br/>es razonable"]
    G -- No --> I["Evaluar cambiar<br/>de proveedor"]

    style D fill:#c8e6c9,stroke:#2e7d32,color:#fff
