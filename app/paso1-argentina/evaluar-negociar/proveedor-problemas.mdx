...
```mermaid
graph TD
    A["Tu negocio depende<br/>de 1 proveedor"] --> B{"¿Qué pasa si falla?"}
    B --> C["Sin stock<br/>Sin ventas<br/>Sin ingresos"]

    D["Tu negocio tiene<br/>2-3 proveedores"] --> E{"¿Qué pasa si uno falla?"}
    E --> F["Activás el alternativo<br/>Seguís vendiendo<br/>Buscás reemplazo con calma"]
```
...
```mermaid
graph LR
    A["Identificar<br/>riesgos"] --> B["Evaluar<br/>impacto"]
    B --> C["Planificar<br/>contingencia"]
    C --> D["Monitorear<br/>activamente"]
    D --> E["Actuar si<br/>hay problemas"]
    E --> A
```
