...
graph TD
    ...
    B -- "Sí" --> C["RECHAZÁ todo el lote<br/>Exigí devolución total"]
    B -- "No" --> D{"¿Tasa de defectos?"}
    D -- "Menos del 5%" --> E["Aceptable<br/>Separá defectuosos<br/>Pedí crédito por esas unidades"]
    D -- "5% a 20%" --> F["Problema moderado<br/>Negociá descuento o<br/>reemplazo parcial"]
    D -- "Más del 20%" --> G["Problema grave<br/>Negociá reemplazo<br/>total o devolución"]
    G --> H{"¿La fábrica responde?"}
    F --> H
    H -- "Sí, coopera" --> I["Acordá solución<br/>por escrito"]
    H -- "No responde" --> J["Evaluá si seguís<br/>con este proveedor"]
```
