...
```mermaid
graph TD
    A["Tenés el CUIT<br/>de la fábrica?"] -- Sí --> B["CUIT activo<br/>en ARCA?"]
    A -- No lo da --> Z["DESCARTÁ<br/>esta fábrica"]
    B -- Sí --> C["Emite factura?"]
    B -- No / Baja --> Z
    C -- Sí --> D["Googlear:<br/>sin reclamos graves?"]
    C -- No --> Z
    D -- Sin problemas --> E["Domicilio<br/>verificable?"]
    D -- Muchos reclamos --> Z
    E -- Sí --> F["AVANZAR<br/>a evaluación"]
    E -- No --> G["Proceder<br/>con precaución"]

    style Z fill:#ffcdd2,stroke:#c62828,color:#fff
    style F fill:#c8e6c9,stroke:#2e7d32,color:#fff
    style G fill:#fff9c4,stroke:#f9a825,color:#fff
```
