...
```mermaid
graph TD
    A["Estimar ventas mensuales<br/>(ej: 30 uds/mes)"] --> B["Cuenta 1<br/>30 x 2.5 meses = 75 uds"]
    C["Capital disponible<br/>(ej: ARS 300,000)"] --> D["Cuenta 2<br/>300,000 / 2,500 = 120 uds"]
    B --> E{"Elegir el MENOR"}
    D --> E
...
```mermaid
graph TD
    A["Cuanto capital tengo?"] --> B{"Mas de ARS 100,000<br/>para un producto?"}
    B -- No --> C["Compra stock disponible<br/>sin MOQ, cantidad chica"]
    B -- Si --> D["Estimo ventas mensuales<br/>(investigar MercadoLibre)"]
    D --> E["Calculo Cuenta 1<br/>Ventas x 2.5 meses"]
    D --> F["Calculo Cuenta 2<br/>Capital / Costo unitario"]
    E --> G{"Elijo el MENOR<br/>entre ambas cuentas"}
    F --> G
    G --> H{"Es mayor al MOQ<br/>de la fabrica?"}
    H -- Si --> I["Pedi esa cantidad"]
    H -- No --> J["Negocia MOQ menor<br/>o busca otra fabrica"]

    style A fill:#2196f3,stroke:#1565c0,color:#fff
    style G fill:#ff9800,stroke:#ef6c00,color:#fff
    style I fill:#4caf50,stroke:#2e7d32,color:#fff
    style C fill:#ff9800,stroke:#ef6c00,color:#fff
    style J fill:#ffebee,stroke:#c62828,color:#fff
```
