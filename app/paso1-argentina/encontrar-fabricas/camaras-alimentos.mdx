...
```mermaid
graph TD
    A["Que alimento<br/>buscas?"] --> B["Contactar COPAL<br/>para derivacion"]
    B --> C["Camara especifica<br/>del sub-sector"]
    C --> D["Pedir directorio<br/>de fabricantes"]
    D --> E{"Fabricante tiene<br/>RNE y RNPA?"}
    E -- Si --> F["Verificar en<br/>ANMAT/SENASA"]
    E -- No --> G["DESCARTAR<br/>este fabricante"]
    F --> H{"Habilitaciones<br/>vigentes?"}
    H -- Si --> I["AVANZAR<br/>a contacto"]
    H -- No --> G

    style G fill:#ffcdd2,stroke:#c62828,color:#fff
    style I fill:#c8e6c9,stroke:#2e7d32,color:#fff
```
