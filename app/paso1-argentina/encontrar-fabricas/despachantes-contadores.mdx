...
```mermaid
graph TD
    A["Vos (comprador)"] --> B["Contacto<br/>profesional"]
    B --> C["Fábrica que<br/>buscás"]

    B1["Despachante<br/>de aduana"] --> C
    B2["Contador /<br/>Estudio contable"] --> C
    B3["Transportista /<br/>Fletero"] --> C
    B4["Proveedor de<br/>packaging"] --> C
    B5["Agente<br/>inmobiliario industrial"] --> C
...
```mermaid
graph TD
    A["Encontrar fábricas<br/>por Google/ML/B2B"] --> B["Verificar con<br/>registros oficiales"]
    B --> C["Preguntar a contactos<br/>si conocen esas fábricas"]
    C --> D{"Te confirman<br/>buena reputación?"}
    D -->|"Sí"| E["Contactar con<br/>referencia del contacto"]
    D -->|No| F["Buscar alternativas"]
    D -->|"No la conocen"| G["Contactar directo<br/>pero con cautela"]
