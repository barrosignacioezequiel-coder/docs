...
```mermaid
graph TD
    A["Que producto buscas?"] --> B{"Es metalurgico<br/>o metalmecanico?"}
    B -- Si --> C["Contactar ADIMRA"]
    B -- No --> D{"Es alimento<br/>o agroindustrial?"}
    D -- Si --> E["Contactar Consejo<br/>Agroindustrial"]
    D -- No --> F{"Tu fabrica ideal<br/>es PyME?"}
    F -- Si --> G["Contactar CAME"]
    F -- No / No se --> H["Contactar UIA"]
    C --> I["Pedir directorio<br/>de asociados"]
    E --> I
    G --> I
    H --> I

    style G fill:#c8e6c9,stroke:#2e7d32,color:#fff
