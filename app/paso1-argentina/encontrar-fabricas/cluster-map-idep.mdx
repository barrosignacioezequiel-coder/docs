...
```mermaid
graph TD
    A["Clúster Industrial"] --> B["Grupo de empresas<br/>del mismo rubro"]
    A --> C["Concentradas en<br/>una misma zona"]
    A --> D["Comparten proveedores<br/>y servicios"]

    B --> E["Ej: fábricas de calzado<br/>en Buenos Aires"]
    C --> F["Ej: polo metalúrgico<br/>en Córdoba"]
    D --> G["Ej: parque industrial<br/>textil en Tucumán"]
...
```mermaid
graph TD
    A["Qué necesitás<br/>encontrar?"] --> B{"Fábricas en<br/>todo el país?"}
    A --> C{"Fábricas en<br/>una zona?"}
    A --> D{"Fábricas<br/>verificadas?"}

    B -->|Si| E["Guía Industrial<br/>+ IDEAR"]
    C -->|Si| F["Cluster Map<br/>+ Dir. Provincial"]
    D -->|Si| G["ArgentinaTradeNet<br/>+ Cámaras"]
